 ...
 */
class Migrate_ extends BaseCommand
{
     ...
     * @var string
     */
    protected $usage = 'migrate_ [options]';
    ...
    public function run(array $params)
    {
        ...
        $database  = $params['d'] ?? CLI::getOption('d');

        if ($database === true || $database === null) $database = 'default';

        $db = Database::connect($database, false);
        $db->initialize();

        $runner = Services::migrations(null, $db);
